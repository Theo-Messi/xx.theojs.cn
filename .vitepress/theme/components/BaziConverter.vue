<template>
    <div class="bazi-converter">
        ...
        <div v-if="bazi">
            <h5>生辰八字</h5>
            <p style="text-align: center; font-size: 1.2rem; font-weight: bold">{{ bazi }}</p>
        </div>
    </div>
</template>
        ...
        const convert = () => {
            if (year.value && month.value && day.value && hour.value !== null) {
                ...
                if (calendarType.value === 'solar') {
                    const solar = Solar.fromYmdHms(year.value, month.value, day.value, hour.value, 0, 0);
                    lunar = Lunar.fromSolar(solar);
                } else {
                    lunar = Lunar.fromYmdHms(year.value, month.value, day.value, hour.value, 0, 0);
                }
